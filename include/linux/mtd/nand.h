...
struct nand_chip {
	...
	int realplanenum;
	int unc_banks;
	int cur_page; /* current read/write page */
	int bbt_sw_rdmz; /* a mode to read bbt with sw rdmz */
	uint oobblock_K;
	uint oobblock_valid;
	uint erasesize_valid;
	uint l_oobblock;/*logical page size that kernel use*/
	uint l_erasesize;/*logical block size that kernel use*/
};
