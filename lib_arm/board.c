...
#include "../common/wmt_dual_boot/wmt_dual_boot.h"
...
static int init_baudrate(void)
{
	...
	gd->bd->bi_baudrate = gd->baudrate = (tmp != NULL)
...
extern int check_udc_to_pc(int force);
extern int check_mptool(void);
extern int run_fastboot(int backlight_on);
extern int set_charge_current(void);
extern int low_power_detect(void);
extern int play_charge_animation(void);
extern void do_wmt_poweroff(void);
extern int enter_fastboot_by_press_key(void);
extern int enter_fastboot_by_special_file(int in_sdcard);

extern void vt1603_snd_init(void);
...
void start_armboot(void)
{
	...
	unsigned long addr;
#endif
	int ret, skip_play_animation, check_result;
...
#	endif
	...
	//********add for vt1603 hp spk always on :av out noisy 2014-4-28
	vt1603_snd_init();
	//*****add end
	if (tstc()) {
	...
	if(g_abort == 0) {
		if(enter_fastboot_by_press_key() == 0)
			g_abort = 1;
	}
	...
	if(g_abort == 0) {
		if(skip_play_animation == 0) {
			if(((s = getenv(ENV_CHARGE_ANIMATION)) != NULL && !strcmp(s, "0"))
			    ...
			    || (!wmt_is_dc_plugin() && (REG8_VAL(POWERUP_SOURCE_STATUS) & PWRBTN_BIT))
...
//#endif
	...
	if(g_abort == 0) {
		char *p_install_dev;
		if((p_install_dev = getenv("wmt.install.dev")) != NULL && strcmp(p_install_dev, "usb") == 0) {
			if (run_command("usb reset", 0) != -1 ) {
				if(run_command(CMD_LOAD_USB_SCRIPT, 0) != -1) {
					...
					if(enter_fastboot_by_special_file(0) == 0)
						g_abort = 1;
				}
			} else
				g_load_script_success = 0;
			...
			if (run_command("mmcinit 0", 0) != -1 ) {
				if(run_command(CMD_LOAD_SCRIPT, 0) != -1) {
					...
					g_load_script_success = 0;
					if(enter_fastboot_by_special_file(1) == 0)
						g_abort = 1;
				}
			} else
				g_load_script_success = 0;
	...
	if(g_abort == 0) {
		...
		if(skip_play_animation == 0) {
			...
			//check mptool if play animation fail
			if(ret != 0) {
				check_result = check_mptool();
				if(check_result == 1) {
					udc_fastboot_exit();
					run_fastboot(0);
				}
			...
			//check mptool if don't play animation
