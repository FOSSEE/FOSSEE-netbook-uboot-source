...
static unsigned int oldUdcPdmaAddrLI, oldUdcPdmaAddrSI;
static unsigned int oldUdcPdmaAddrLO, oldUdcPdmaAddrSO;
...
static const u8 string_desc_3[] = { // STRING_SERIAL
	(0x18 + 2), 0x03,
	'0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, 'W', 0x0,
	'M', 0x0, '8', 0x0, '8', 0x0, '8', 0x0, '0', 0x0
};

static const u8 string_detect_mptool[] = { // STRING_SERIAL
	(0x18 + 2), 0x03,
	'0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, '0', 0x0, 'D', 0x0,
	'E', 0x0, 'T', 0x0, 'E', 0x0, 'C', 0x0, 'T', 0x0
};
		...
		case USB_REQ_GET_DESCRIPTOR:
			...
			case USB_DT_STRING:
				...
				case STRING_SERIAL:
					...
					if(detect_mptool) {
						if(value > sizeof(string_detect_mptool))
							value = sizeof(string_detect_mptool);
						memcpy((unsigned char *)control_buf, (u8 *)string_detect_mptool, sizeof(string_detect_mptool));
						printf_string_serial((u8 *)string_detect_mptool, sizeof(string_detect_mptool));
					} else {
...
extern void usbd_dealloc_urb (struct urb *urb);
...
static void wmt_pdma_init(void)
{
	...
	/*descriptor locate*/
	oldUdcPdmaAddrLI = (unsigned int)malloc(0x120);//dma_alloc_coherent(pDMADescLI, 0x100, &UdcPdmaPhyAddrLI, GFP_KERNEL|GFP_ATOMIC);
	oldUdcPdmaAddrSI = (unsigned int)malloc(0x120);//dma_alloc_coherent(pDMADescSI, 0x100, &UdcPdmaPhyAddrSI, GFP_KERNEL|GFP_ATOMIC);
	oldUdcPdmaAddrLO = (unsigned int)malloc(USB_DESCRIPT_LO_SIZE + 0x20); //dma_alloc_coherent(pDMADescLO, 0x100, &UdcPdmaPhyAddrLO, GFP_KERNEL|GFP_ATOMIC);
	oldUdcPdmaAddrSO = (unsigned int)malloc(0x120);//dma_alloc_coherent(pDMADescSO, 0x100, &UdcPdmaPhyAddrSO, GFP_KERNEL|GFP_ATOMIC);
	UdcPdmaAddrLI = (oldUdcPdmaAddrLI + 0x20) & ~0x1F;
	UdcPdmaAddrSI = (oldUdcPdmaAddrSI + 0x20) & ~0x1F;
	UdcPdmaAddrLO = (oldUdcPdmaAddrLO + 0x20) & ~0x1F;
	UdcPdmaAddrSO = (oldUdcPdmaAddrSO + 0x20) & ~0x1F;
...
int udc_deinit(void)
{
	if(bulk_out_dma1 != NULL) {
		free(bulk_out_dma1);
		bulk_out_dma1 = NULL;
	}

	if(bulk_out_dma2 != NULL) {
		free(bulk_out_dma2);
		bulk_out_dma2 = NULL;
	}

	if(bulk_in_dma1 != NULL) {
		free(bulk_in_dma1);
		bulk_in_dma1 = NULL;
	}

	if(bulk_in_dma2 != NULL) {
		free(bulk_in_dma2);
		bulk_in_dma2 = NULL;
	}
	...
	if(oldUdcPdmaAddrLI != 0) {
		free((void *)oldUdcPdmaAddrLI);
		oldUdcPdmaAddrLI = 0;
	}

	if(oldUdcPdmaAddrSI != 0) {
		free((void *)oldUdcPdmaAddrSI);
		oldUdcPdmaAddrSI = 0;
	}

	if(oldUdcPdmaAddrLO != 0) {
		free((void *)oldUdcPdmaAddrLO);
		oldUdcPdmaAddrLO = 0;
	}

	if(oldUdcPdmaAddrSO != 0) {
		free((void *)oldUdcPdmaAddrSO);
		oldUdcPdmaAddrSO = 0;
	}
...
int usb_plugin(void)
{
	...
	if(!(pDevReg->SelfPowerConnect & 0x04))
		pDevReg->SelfPowerConnect |= 0x04;

	return (pDevReg->SelfPowerConnect & 0x01);
}
