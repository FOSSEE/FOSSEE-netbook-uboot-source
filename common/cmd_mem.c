...
int do_mem_cp ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	*/
	...
	if (size == 15) {
		...
		if ((addr > 0xffffffff) || (addr < ((0xffffffff-info->size) + 1))) {
			printf ("sf offset is invalid 0x%x \n", addr);
			return 1;
		}
		//printf ("sf_ofs=0x%x, mem_ofs=0x%x, sf_len=0x%x\n", addr, dest, count);
		read_buff(info, addr, dest, count);*/
		return 0;
	}
