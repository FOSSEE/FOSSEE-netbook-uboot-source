...
#ifdef CMD_SAVEENV
env_t *flash_addr = (env_t *)CFG_ENV_ADDR;
#endif
env_t *flash_addr2 = (env_t *)(CFG_ENV_ADDR + CFG_ENV_SIZE);
env_t *env_ptr2 = (env_t *)(CFG_ENV_ADDR + CFG_ENV_SIZE);
...
int  env_init(void)
{
	...
	if (flag1 == ACTIVE_FLAG) {
		...
		if (crc1_ok) {
			...
			if (crc2_ok = crc32(0, flash_addr_new->data, ENV_SIZE) == flash_addr_new->crc) {
				/*printf("crc2_ok\n");*/
			} else {
				/*printf("!crc1_ok && !crc2_ok\n");*/
			}			
		}
	} else {
		memcpy(env_ptr, (void*)flash_addr_new, CFG_ENV_SIZE);
		crc2_ok = (crc32(0, env_ptr->data, ENV_SIZE) == env_ptr->crc);
		if(crc2_ok) {
			/*printf("crc2_ok, flsg1 = OBSOLETE_FLAG\n");*/
		} else {
			if(crc1_ok = (crc32(0, flash_addr->data, ENV_SIZE) == flash_addr->crc)) {
				/*printf("crc2_ok, flsg1 = OBSOLETE_FLAG\n");*/
			} else {
				/*printf("!crc1_ok flsg1 = OBSOLETE_FLAG && !crc2_ok\n");*/
			}
		}
	}
...
int saveenv(void)
{
	...
	debug (" %08lX ... %08lX ...",
		...
		sizeof(env_new.data)+(ulong)&(flash_addr_new->data));
	if ((rc = flash_write((char *)env_new.data,
			(ulong)&(flash_addr_new->data),
			sizeof(env_new.data))) ||
	    (rc = flash_write((char *)&(env_new.crc),
