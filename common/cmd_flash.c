...
int do_sfread ( cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	ulong sf_ofs, mem_ofs, sf_len;
	flash_info_t *info;
	info = &flash_info[0];
	...
	sf_ofs = simple_strtoul(argv[1], NULL, 16);
	mem_ofs = simple_strtoul(argv[2], NULL, 16);
	sf_len  = simple_strtoul(argv[3], NULL, 16);
	if ((sf_ofs > 0xffffffff) || (sf_ofs < ((0xffffffff-info->size) + 1))) {
		printf ("sf offset is invalid 0x%x \n", sf_ofs);
		return 1;
	}
	printf ("sf_ofs=0x%x, mem_ofs=0x%x, sf_len=0x%x\n", sf_ofs, mem_ofs, sf_len);
	read_buff(info, sf_ofs, mem_ofs, sf_len);
	return 0;
}
	...
	if (strcmp(argv[2], "all") == 0) {
		for (bank=1; bank<=CFG_MAX_FLASH_BANKS; ++bank) {
			...
			printf ("%sProtect Flash Bank # %ld\n",
				...
				if (flash_real_protect(info, bank-1, p))
					rcode = 1;
				putc ('.');
#else
