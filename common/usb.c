...
#include <u-boot/unaligned.h> //Charles
...
int usb_new_device(struct usb_device *dev)
{
	...
	USB_PRINTF("Charles:dev->descriptor.bDeviceClass 0x%x\n", dev->descriptor.bDeviceClass);

	/* find the port number we're at */
	if (parent) {
		int j;
	USB_PRINTF("parent 0x%x\n", parent);
	...
	USB_PRINTF("new device dev->descriptor.idVendor=0x%x, dev->descriptor.idProduct=0x%x\n",
	dev->descriptor.idVendor, dev->descriptor.iProduct	);
...
{
	unsigned char buffer[USB_BUFSIZ], *bitmap, status = 0;
	...
	memcpy((unsigned char *)&hub->desc, buffer, 9);
	/* adjust 16bit values */
	...
	for (i = 0; i < dev->maxchild; i++) {
		...
		if (portchange & USB_PORT_STAT_C_CONNECTION) {
			...
			status = usb_hub_port_connect_change(dev, i);
			if (status == -1) 
				return -1;
		}
