...
/****************************************************************
Function:
    According to the parlist and the Cylinder paramters, to adjust the address and length of each part.
Parameters:
input:
    partlist: is the length size list which the user want. the unit is Mega byte.
    partnum: how many parts the user want
output:
    address: is a address list, and each is in  sector byte(512B) unit.
    length: is a length list, and each is in sector byte(512B) unit.
return:
*******************************************************************/
int getUserOrderParam(block_dev_desc_t * dev_desc, int *partlist, int *address, int *length, int partnum)
{
      int i=0;
	//unsigned char buffer[DEFAULT_SECTOR_SIZE];
    	//find the geometry
	...
	g_cylinders = dev_desc->lba / (g_heads * g_sectors );
      ...
      for(i=0;i<partnum;i++)
	  	length[i]=partlist[i]*1024*2;


	//adjust the length in cylinder unit
	address[0]=0x3f;
	if ((address[0]+length[0])%(g_sectors*g_heads)) {
		length[0]+=g_sectors*g_heads-((length[0]+address[0])%(g_sectors*g_heads));
	}

	for(i=1;i<3;i++) {
		address[i]=address[i-1]+length[i-1];
		if ((address[i]+length[i])%(g_sectors*g_heads)) {
			length[i]+=g_sectors*g_heads-((length[i]+address[i])%(g_sectors*g_heads));
		}
	}

	//extended partition
	address[3]=address[2]+length[2];
	length[3]=dev_desc->lba-address[3]-g_sectors*g_heads;
	if ((address[3]+length[3])%(g_sectors*g_heads)) {
		length[3]+=g_sectors*g_heads-((length[3]+address[3])%(g_sectors*g_heads));
	}

	//logical partitions
       address[4]=address[3];
	if ((length[4]+0x3f)%(g_sectors*g_heads)) {
		length[4]+=g_sectors*g_heads-((length[4]+0x3f)%(g_sectors*g_heads));
	}
	for(i=5;i<partnum-1;i++) {
		address[i]=address[i-1]+length[i-1]+0x3f;
		if ((0x3f+length[i])%(g_sectors*g_heads)) {
			length[i]+=g_sectors*g_heads-((0x3f+length[i])%(g_sectors*g_heads));
		}
	}
	if (partlist[i]==0xffffffff) {
		address[i]=address[i-1]+length[i-1]+0x3f;
		length[i]= dev_desc->lba-address[i]-g_sectors*g_heads;
		if ((0x3f+length[i])%(g_sectors*g_heads)) {
			length[i]+=g_sectors*g_heads-((length[i]+0x3f)%(g_sectors*g_heads));
		}
	} else {
             	address[i]=address[i-1]+length[i-1]+0x3f;
		if ((0x3f+length[i])%(g_sectors*g_heads)) {
	...
	for(i=0;i<partnum;i++)
		printf("the ordered parts are like these (512Byte unit: addr %x, length %x\n",
		    address[i], length[i]);
}
